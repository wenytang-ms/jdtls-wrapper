...
Run: `npx --yes @vscjava/java-language-server --version`
...
```
npm install -g @vscjava/java-ls-jre-win32-x64    # Windows
npm install -g @vscjava/java-ls-jre-linux-x64     # Linux
npm install -g @vscjava/java-ls-jre-darwin-arm64   # macOS ARM
```

### 2. jdtls Server Files Missing

If `npx @vscjava/java-language-server --stdio` fails with "launcher jar not found":

```bash
# Download jdtls server files
cd $(npm root -g)/@vscjava/java-language-server
node ../../scripts/download-jdtls.js
```

### 3. Re-verify

Run `npx --yes @vscjava/java-language-server --version` again to confirm the fix worked.
